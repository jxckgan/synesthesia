...
class UDPDiscoveryTransport : public ITransport {
public:
    explicit UDPDiscoveryTransport(uint16_t port = 19851, bool is_server = false);
    ~UDPDiscoveryTransport() override;
...
public:
    static std::unique_ptr<ITransport> createMainTransport(const std::string& endpoint, bool is_server);
    static std::unique_ptr<ITransport> createDiscoveryTransport(uint16_t port, bool is_server);
};
